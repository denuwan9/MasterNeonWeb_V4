...
const SiteLayout = () => {
  ...
  const pageVariants = {
    ...
    animate: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.45, ease: [0.4, 0, 0.2, 1] as [number, number, number, number] } },
    exit: { opacity: 0, y: -6, scale: 0.995, transition: { duration: 0.35, ease: [0.4, 0, 1, 1] as [number, number, number, number] } },
  }
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#1f2937_0%,#05060a_55%)] text-white">
      ...
      <div className="relative">
        <motion.div
          ...
          transition={{ duration: isRouteChanging ? 0.55 : 0.35, ease: [0.4, 0, 0.2, 1] as [number, number, number, number] }}
